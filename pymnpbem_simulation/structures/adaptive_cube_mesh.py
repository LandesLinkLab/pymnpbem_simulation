...
def _make_face_grid(axis: int, sign: int, half: float, n: int,
    ...
    # The (u, v) grid is laid out as (y, z) on an x-face, (x, z) on a y-face
    # and (x, y) on a z-face. Since x_hat cross z_hat = -y_hat, the y-face grid
    # is left-handed with respect to its own outward direction while the other
    # two are right-handed, so applying the same winding to all three flips
    # both y-face normals inward. Measured before this correction: 512 of 768
    # elements outward (+y and -y groups inverted), 64 badly-oriented directed
    # edges, and a signed volume of 8339 against 24885 for the equivalent
    # tricube. Fold that handedness into the winding decision.
    handed = -1 if axis == 1 else 1
...
def build_adaptive_cube(
    ...
    _assert_watertight(verts_cat, faces_cat, face_densities, edge_profile_kwargs)

    return Particle(verts_cat, faces_cat, interp = safe_interp)


def _assert_watertight(verts: np.ndarray,
        faces: np.ndarray,
        face_densities: Optional[Dict[str, int]],
        edge_profile_kwargs: Optional[Dict[str, Any]]) -> None:
    """Refuse to hand back a surface with holes.

    Mixing per-face densities leaves T-junctions where a finely divided face
    meets a coarse one (measured: 144 unmatched edges for +z=16 / -z=4 at
    n_default=8), and the cube edge profile displaces shared vertices by
    different amounts depending on which face processes them, so `_merge_verts`
    can no longer weld them (192 unmatched edges at e=5.0). Either way the
    boundary is not closed and the BEM interior/exterior assignment is
    undefined, so this is an error rather than a warning. The uniform path is
    unaffected (all edges matched).
    """
    ...
    if face_densities:
        culprits.append('<face_densities> (per-face density leaves T-junctions '
                'where divisions differ)')

    if edge_profile_kwargs:
        culprits.append('<edge_profile> (shared vertices are displaced '
                'inconsistently and can no longer be welded)')

    raise ValueError(
        '[error] build_adaptive_cube produced a non-watertight surface: {} '
        'unmatched edges. Cause: {}. Use a uniform <mesh_density>/<n_per_edge> '
        'instead. (닫히지 않은 메시 — BEM 안팎 배정이 정의되지 않습니다.)'.format(
            n_open, '; '.join(culprits) if culprits else 'unknown'))
