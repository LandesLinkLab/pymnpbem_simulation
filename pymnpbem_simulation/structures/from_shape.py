...
def _voxels_to_mesh(
    ...
    if method == 'cube':
        # _voxels_to_cube_mesh emits all 12 triangles of every voxel, including
        # the faces shared with a neighbouring voxel. Those interior faces have
        # the same material on both sides, so the result is not a boundary
        # surface at all: measured on a 4x4x4 block, 768 faces of which ~75 %
        # are internal, 396 non-manifold edges, and only 50 % outward normals
        # (vs 192 faces, 0 non-manifold, 100 % outward for 'surface').
        raise ValueError(
            '[error] <voxel_method>="cube" emits interior voxel faces and does '
            'not form a valid BEM boundary (non-manifold, ~75% internal faces). '
            'Use "surface", which keeps only the outer boundary. '
            '(내부 면까지 내보내 BEM 경계로 유효하지 않습니다 — "surface" 를 쓰세요.)')
