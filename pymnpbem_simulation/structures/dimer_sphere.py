...
class DimerSphereBuilder(StructureBuilder):
    ...
    def build(self) -> Tuple[Any, Any, int]:
        ...
        n_verts = _resolve_sphere_n(self.cfg_struct, diameter)
        ...
        # Disconnected particles need distinct epstab entries (MNPBEM demos).
        epstab, inout = replicate_inout(epstab, [[2, 1]], 2)

        p = ComParticle(epstab, [s1, s2], inout,
