...
from .sphere import replicate_inout
...
class WithNonlocalBuilder(StructureBuilder):
    ...
    def build(self) -> Tuple[Any, Any, int]:
        ...
        # With more than one sub-particle each disconnected core/shell pair
        # needs its own epstab entries — see sphere.replicate_inout.
        epstab = [eps_embed, eps_metal_core, eps_shell]
        epstab, inout_rows = replicate_inout(
                epstab, [[3, 1], [2, 3]], len(p_base.p))
